{
	...
	vector<chi> chiTable;
	for(double epsim = 1.5; epsim <= 2.5; epsim += 0.001)
	{
		cout << "r = " << r << endl;
			...
			*/
			...
			for(int i =0 ; i < extJC.size() ; i++)
			{
				if(extJC[i].l >= 500 && extJC[i].l <= 550)
				{
					entry.chi2 += pow(extJC[i].sig*factor - assSpet[i].sig,2);
					m++;
				}
			}
			//entry.chi2 /= (m);
	...
	int k = 0;
	ofstream outf("chi2_epsim.csv");
	for(int i =0 ; i < chiTable.size() ; i++)
	{
		outf << chiTable[i].epsim << "," << chiTable[i].chi2 << endl;
		gr->SetPoint(k++, chiTable[i].epsim, chiTable[i].chi2);
	}
	gr->Fit("pol2","M");
	gr->SetMarkerStyle(3);
	gr->Draw("ap");
